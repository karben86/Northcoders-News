...
class Article extends Component {
      ...
      this.setState((currState) => {
        return {
          comments: [...currState.comments].filter(
            (comment) => comment.comment_id !== +event.target.id
          ),
        };
      });
    });
  ...
  render() {
    let { article, comments, err, isLoading } = this.state;
    article = article[0];
    return (
      <main>
        {isLoading ? null : err ? (
          ...
          <div>
            ...
            <p>
              <b>
                ...
                | Author: {article.author} | Votes: {article.votes} | Comment
                Count: {article.comment_count} | Date:{" "}
            ...
            <ul>
              {comments.map((comment) => (
                <li key={comment.comment_id}>
                  <p>{comment.body}</p>
                  Author: {comment.author} | Votes: {comment.votes} | Date:{" "}
                  {comment.created_at.slice(0, 10)}
                  <br></br>
                  {comment.author === "jessjelly" ? (
                    <button id={comment.comment_id} onClick={this.handleDelete}>
                      Delete Comment
                    </button>
                  ) : null}
                  ...
                  <button
                    ...
                    onClick={(event) => this.voteClick(comment.comment_id, 1)}
                    disabled={this.state.buttonDisabled[comment.comment_id]}
                  >
                    👆
                  </button>
                  <button
